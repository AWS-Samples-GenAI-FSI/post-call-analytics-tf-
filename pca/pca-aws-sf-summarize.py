...
def get_templates_from_dynamodb():
    templates = []
    try:
        # Use the simple LLM table with single key structure
        simple_table_name = LLM_TABLE_NAME + '-simple'
        SUMMARY_PROMPT_TEMPLATE = dynamodb_client.get_item(
            Key={'LLMPromptTemplateId': {'S': 'LLMPromptSummaryTemplate'}},
        ...
        if 'LLMPromptTemplateValue#Summary' in prompt_templates:
            prompt = prompt_templates['LLMPromptTemplateValue#Summary']['S'].replace("<br>", "\n")
